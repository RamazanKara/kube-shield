...
var builtInRules = []Rule{
	{
		...
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Baseline host namespaces"}, {Name: "CIS Kubernetes Benchmark", Control: "4.2.2"}},
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Baseline host namespaces"}, {Name: "CIS Kubernetes Benchmark", Control: "4.2.3"}},
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Baseline host namespaces"}, {Name: "CIS Kubernetes Benchmark", Control: "4.2.4"}},
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.1.6"}},
	...
	},
	{
		...
		References:     []string{"https://kubernetes.io/docs/concepts/security/pod-security-standards/#baseline"},
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Baseline privileged containers"}, {Name: "CIS Kubernetes Benchmark", Control: "4.2.1"}},
	...
	},
	{
		...
		References:     []string{"https://kubernetes.io/docs/tasks/configure-pod-container/security-context/"},
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Restricted running as non-root"}, {Name: "CIS Kubernetes Benchmark", Control: "4.2.6"}},
	...
	},
	{
		...
		References:     []string{"https://kubernetes.io/docs/tasks/configure-pod-container/security-context/"},
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Restricted privilege escalation"}},
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "Pod Security Standards", Control: "Restricted capabilities"}},
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.5.2"}},
...
var cisRules = []Rule{
	rule("CIS-4.1.1", "cis", CategoryCIS, "cluster-admin role bound to ServiceAccount", SeverityCritical, ConfidenceHigh, "Cluster-admin grants full cluster control to a workload identity.", "Compromise of the bound ServiceAccount can compromise the cluster.", "Create a more restrictive role and bind that role instead of cluster-admin.", "4.1.1"),
	rule("CIS-4.1.2", "cis", CategoryCIS, "ClusterRole with secret access", SeverityHigh, ConfidenceHigh, "Cluster-wide secret read access exposes sensitive data across namespaces.", "A compromised principal can read secrets outside its intended scope.", "Restrict secret access to namespace-scoped Roles where possible.", "4.1.2"),
	...
	rule("CIS-4.1.6", "cis", CategoryCIS, "Default SA automounts token", SeverityMedium, ConfidenceMedium, "Ambient default service account tokens increase API credential exposure.", "Compromised pods may gain Kubernetes API access.", "Disable automounting or use dedicated least-privilege ServiceAccounts.", "4.1.6"),
	rule("CIS-4.2.1", "cis", CategoryCIS, "Privileged container", SeverityCritical, ConfidenceHigh, "Privileged containers bypass normal container isolation.", "Compromise can lead to node compromise.", "Remove privileged: true and grant only required privileges.", "4.2.1"),
	rule("CIS-4.2.2", "cis", CategoryCIS, "Pod uses hostPID", SeverityHigh, ConfidenceHigh, "Host PID namespace sharing exposes host processes.", "Workloads can inspect host and peer workload processes.", "Set hostPID to false or remove the field.", "4.2.2"),
	rule("CIS-4.2.3", "cis", CategoryCIS, "Pod uses hostIPC", SeverityHigh, ConfidenceHigh, "Host IPC namespace sharing exposes host IPC primitives.", "Workloads can interact with host shared memory resources.", "Set hostIPC to false or remove the field.", "4.2.3"),
	rule("CIS-4.2.4", "cis", CategoryCIS, "Pod uses hostNetwork", SeverityHigh, ConfidenceHigh, "Host networking bypasses pod network isolation.", "Workloads gain direct access to host network interfaces.", "Set hostNetwork to false or remove the field.", "4.2.4"),
	rule("CIS-4.2.6", "cis", CategoryCIS, "Container may run as root", SeverityHigh, ConfidenceMedium, "Containers without non-root enforcement may run as UID 0.", "Root execution increases impact after compromise.", "Set runAsNonRoot: true and a non-zero runAsUser.", "4.2.6"),
	rule("CIS-4.2.9", "cis", CategoryCIS, "Container has added capabilities", SeverityMedium, ConfidenceHigh, "Added capabilities expand Linux privileges beyond defaults.", "Compromise can gain capabilities unnecessary for the workload.", "Drop unnecessary capabilities and add only the minimum required.", "4.2.9"),
	rule("CIS-4.3.1", "cis", CategoryCIS, "No network policy in namespace", SeverityHigh, ConfidenceHigh, "Namespaces without NetworkPolicy commonly allow unrestricted pod traffic.", "A compromised pod can more easily reach peer workloads.", "Create default-deny policies and allow required traffic explicitly.", "4.3.1"),
	rule("CIS-4.4.1", "cis", CategoryCIS, "Secret exposed as env var", SeverityMedium, ConfidenceHigh, "Secrets in environment variables can leak through process metadata, logs, or crash reports.", "Sensitive values become harder to rotate and contain.", "Mount secrets as files instead of environment variables.", "4.4.1"),
	rule("CIS-4.5.1", "cis", CategoryCIS, "No resource quotas in namespace", SeverityLow, ConfidenceHigh, "Namespaces without quotas do not bound aggregate resource usage.", "Workloads can consume excessive namespace resources.", "Create ResourceQuota objects for shared namespaces.", "4.5.1"),
	rule("CIS-4.5.2", "cis", CategoryCIS, "No LimitRange in namespace", SeverityLow, ConfidenceHigh, "Namespaces without LimitRange do not define default or bounded per-container resources.", "Workloads may run without expected resource constraints.", "Create LimitRange objects with appropriate defaults and limits.", "4.5.2"),
}
...
var rbacRules = []Rule{
	{
		...
		Standards:   []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.1 RBAC"}},
		DataAccess:  DataAccessObjectSpec, DefaultEnabled: true,
	},
	{
	...
	},
	{
		...
		Standards:      []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.1.2"}},
	...
	},
	{
		...
		References:  []string{"https://kubernetes.io/docs/reference/access-authn-authz/rbac/"},
		Standards:   []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.1.1"}},
		DataAccess:  DataAccessObjectSpec, DefaultEnabled: true,
	},
	{
...
var netpolRules = []Rule{
	{
		...
		Standards:      []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.3.1"}},
...
var secretRules = []Rule{
	{
		...
		Standards:   []StandardMapping{{Name: "CIS Kubernetes Benchmark", Control: "4.4.1"}},
		DataAccess:  DataAccessObjectSpec, DefaultEnabled: true,
	},
	{
